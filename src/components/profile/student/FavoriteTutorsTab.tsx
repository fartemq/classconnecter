...
// Компонент карточки репетитора для отображения в сетке
const TutorGridCard: React.FC<{ tutor: FavoriteTutor; onRemove: (id: string, e: React.MouseEvent) => void }> = ({ 
  tutor, 
  onRemove 
}) => {
...
// Компонент карточки репетитора для отображения в списке
const TutorListCard: React.FC<{ tutor: FavoriteTutor; onRemove: (id: string, e: React.MouseEvent) => void }> = ({ 
  tutor, 
  onRemove 
}) => {
  const navigate = useNavigate();
...
export const FavoriteTutorsTab = () => {
  ...
  const handleRemoveFromFavorites = (tutorId: string, e: React.MouseEvent) => {
    ...
    setFavoriteTutors(prevTutors => prevTutors.filter(tutor => tutor.id !== tutorId));
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      <Tabs value={viewMode} onValueChange={(value) => setViewMode(value as "grid" | "list")} className="w-full mb-6">
        ...
        <TabsContent value="grid" className="mt-0">
          {favoriteTutors && favoriteTutors.length > 0 ? (
            ...
            <EmptyState 
              icon={<Heart size={48} />}
              title="У вас пока нет избранных репетиторов"
